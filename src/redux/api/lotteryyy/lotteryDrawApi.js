...
export const lotteryDrawApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders: (headers) => {
      ...
      if (userData) {
        // ✅ FIX: Send lowercase roles to match backend expectations
        const normalizedRoles = (userData.roles || ['Voter']).map(role => {
          // Convert to lowercase
          const lowerRole = role.toLowerCase();
          // Handle special cases
          if (lowerRole === 'contentcreator') return 'content_creator';
          if (lowerRole.includes('individual election creator')) return 'content_creator';
          return lowerRole;
        });
        ...
        headers.set('x-user-data', JSON.stringify({
          ...
          roles: normalizedRoles,  // ✅ Lowercase roles
