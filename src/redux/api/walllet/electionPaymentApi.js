...
export const electionPaymentApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // ✅ FIXED: Pay for election participation with paymentGateway support
    payForElection: builder.mutation({
      query: ({ electionId, regionCode, paymentGateway = 'stripe' }) => { // ← Accept paymentGateway
        console.log('🔵 Calling POST /wallet/pay-election:', { 
          electionId, 
          regionCode, 
          paymentGateway // ← Log it
        });
        return {
          ...
          body: {
            electionId,
            regionCode,
            paymentGateway, // ← CRITICAL: Include in body
          },
        };
      },
...
//last workable code with stripe success
// // src/redux/api/walllet/electionPaymentApi.js
// import { createApi, fetchBaseQuery } from '@reduxjs/toolkit/query/react';
