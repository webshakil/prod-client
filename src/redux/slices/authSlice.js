...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    // ✅ UPDATED: Now also handles sessionFlags from backend
    setUserCheckData: (state, action) => {
      const {
        ...
        isFirstTime,
        sessionId,
        sessionFlags, // ✅ NEW: Flags from backend
      } = action.payload;
      ...
      // ✅ NEW: Set flags from backend (what's already completed in previous sessions)
      if (sessionFlags) {
        if (sessionFlags.userDetailsCollected) {
          state.completedAuthSteps.userDetails = true;
        }
        if (sessionFlags.biometricCollected) {
          state.completedAuthSteps.biometric = true;
        }
        if (sessionFlags.securityQuestionsAnswered) {
          state.completedAuthSteps.securityQuestions = true;
        }
        console.log('🔄 Redux: Session flags from backend:', sessionFlags);
      }
  ...
  state.error = null;
    ...
    setSessionFlags: (state, action) => {
      const {
        ...
        step_number,
        // ✅ Also handle camelCase versions from backend
        userDetailsCollected,
        biometricCollected,
        securityQuestionsAnswered,
      } = action.payload;

      if (user_details_collected !== undefined || userDetailsCollected !== undefined) {
        state.completedAuthSteps.userDetails = user_details_collected || userDetailsCollected;
      }
      if (biometric_collected !== undefined || biometricCollected !== undefined) {
        state.completedAuthSteps.biometric = biometric_collected || biometricCollected;
      }
      if (security_questions_answered !== undefined || securityQuestionsAnswered !== undefined) {
        state.completedAuthSteps.securityQuestions = security_questions_answered || securityQuestionsAnswered;
      }
      if (step_number !== undefined) {
        state.currentAuthStep = step_number;
      }

      console.log('🔄 Redux: Session flags updated', action.payload);
    },
