...
const initialState = {
  ...
  isNewUser: false,
  userVoteCount: 0,
  recommendationType: null, // 'personalized' | 'trending' | 'general'
  ...
  async ({ userId, limit = 10 }, { rejectWithValue }) => {
    try {
      ...
      return {
        ...
        userVoteCount: data.user_vote_count || 0,
        recommendationType: data.recommendation_type || 'general',
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchPersonalizedRecommendations.fulfilled, (state, action) => {
        ...
        state.userVoteCount = action.payload.userVoteCount || 0;
        state.recommendationType = action.payload.recommendationType || 'general';
        state.recommendationMessage = action.payload.message || null;
...
export const selectUserVoteCount = (state) => state.recommendations?.userVoteCount || 0;
export const selectRecommendationType = (state) => state.recommendations?.recommendationType || null;
export const selectRecommendationMessage = (state) => state.recommendations?.recommendationMessage || null;
