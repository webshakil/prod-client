...
export default function EmailVerificationForm({ sessionId, email, onNext }) {
  ...
  const handleSendOTP = async () => {
    try {
      ...
      setOtpSent(true);
      setTimer(60);
      dispatch(setSuccess(t('emailVerification.success.otpSent')));
    } catch (error) {
      const errorMessage = error.data?.message || t('emailVerification.errors.sendFailed');
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      dispatch(setSuccess(t('emailVerification.success.verified')));
      console.log('✅ Email OTP verified, calling onNext()');
      onNext();
    } catch (error) {
      const errorMessage = error.data?.message || t('emailVerification.errors.verifyFailed');
      dispatch(setError(errorMessage));
    }
  };
  ...
  const handleSkipOTP = async () => {
    try {
      ...
      dispatch(setEmailOTPVerified());
      dispatch(setSuccess(t('emailVerification.success.skipped')));
      console.log('✅ Email OTP skipped, calling onNext()');
      onNext();
    } catch (error) {
      const errorMessage = error.data?.message || t('emailVerification.errors.skipFailed');
      dispatch(setError(errorMessage));
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      {!otpSent && (
        ...
        >
          {isSending ? <Loading /> : t('emailVerification.sendOTP')}
        </button>
      )}
      ...
      {otpSent && (
        <form onSubmit={handleVerifyOTP} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {isVerifying ? <Loading /> : t('emailVerification.verifyOTP')}
            </button>
            <button
              type="button"
            ...
            >
              {timer > 0 ? t('emailVerification.resendTimer', { timer }) : t('emailVerification.resend')}
            </button>
          </div>
        </form>
      )}
