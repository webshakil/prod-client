...
export default function SMSVerificationForm({ sessionId, phone, onNext }) {
  ...
  const handleSendOTP = async () => {
    try {
      ...
      setOtpSent(true);
      setTimer(60);
      dispatch(setSuccess(t('phoneVerification.success.otpSent')));
    } catch (error) {
      const errorMessage = error.data?.message || t('phoneVerification.errors.sendFailed');
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      dispatch(setSuccess(t('phoneVerification.success.verified')));
      ...
      const errorMessage = error.data?.message || t('phoneVerification.errors.verifyFailed');
      dispatch(setError(errorMessage));
    }
  };
  ...
  const handleSkipOTP = async () => {
    try {
      ...
      dispatch(setSMSOTPVerified());
      dispatch(setSuccess(t('phoneVerification.success.skipped')));
      console.log('✅ SMS OTP skipped, calling onNext()');
      onNext();
    } catch (error) {
      const errorMessage = error.data?.message || t('phoneVerification.errors.skipFailed');
      dispatch(setError(errorMessage));
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <p className="text-center text-gray-600 mb-6">
        {t('phoneVerification.subtitle')} {phone}
      </p>
      ...
      {!otpSent && (
        ...
        >
          {isSending ? <Loading /> : t('phoneVerification.sendOTP')}
        </button>
      )}
      ...
      {otpSent && (
        <form onSubmit={handleVerifyOTP} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {isVerifying ? <Loading /> : t('phoneVerification.verifyOTP')}
            </button>
            <button
              type="button"
            ...
            >
              {timer > 0 ? t('phoneVerification.resendTimer', { timer }) : t('phoneVerification.resend')}
            </button>
          </div>
        </form>
      )}
