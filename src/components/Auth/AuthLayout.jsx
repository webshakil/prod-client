...
export default function AuthLayout() {
  ...
  const handleUserDetailsNext = () => {
    ...
    console.log('Current session:', auth.sessionId);
    // ✅ Skip Step 5 (Biometric) on web - will be collected on mobile app
    console.log('⏭️ Skipping biometric on web - will be collected in mobile app');
    
    // ✅ Check if security questions already answered (maybe from mobile)
    if (auth.completedAuthSteps?.securityQuestions) {
      console.log('⏭️ Security questions already answered (from mobile) - completing auth');
      completeAuth();
    } else {
      console.log('→ Moving to Step 6 (Security Questions)');
      setLocalStep(6); // Go to Security Questions
    }
  };
  ...
  const handleBiometricNext = () => {
    ...
    // ✅ Check if security questions already answered (maybe from mobile)
    if (auth.completedAuthSteps?.securityQuestions) {
      console.log('⏭️ Security questions already answered - completing auth');
      completeAuth();
    } else {
      setLocalStep(6);
    }
...
//     // ✅ Skip Step 5 (Biometric) on web - will be collected on mobile app
//     console.log('⏭️ Skipping biometric on web - will be collected in mobile app');
//     setLocalStep(6); // Jump directly to Step 6 (Security Questions)
//   };
...
//   );
// }
//last workable code only to avoid biometric in web above code
