...
const VOTING_METHODS = [
  {
    ...
    icon: '🗳️',
    color: 'blue',
    description: 'Single choice selection - most votes wins',
    details: 'Voters select ONE option from the list. Option with most votes wins.',
  ...
  },
  {
    ...
    details: 'Voters rank options by preference (1st, 2nd, 3rd...). Elimination rounds until majority.',
  ...
  },
  {
    ...
    icon: '✅',
    color: 'green',
    description: 'Multiple choice approval selection',
    details: 'Voters approve (Yes/No) multiple options they support.',
    benefits: ['Reduces strategic voting', 'Simple ballot design', 'Encourages moderate choices', 'Express broader preferences']
  }
];

// ✅ CHANGED: Renamed from "Candidate Listing" to "Multiple Choice"
const QUESTION_TYPES = [
  {
    id: 'candidate_listing',
    name: 'Multiple Choice',
    icon: <FaListOl />,
    color: 'blue',
    description: 'List of options for ballot-style voting',
    requiresAnswers: true,
    minAnswers: 2,
    maxAnswers: 100,
    helpText: 'Create a list of all options/choices. Voters will see them as a ballot based on your voting type.'
  },
  {
    id: 'comparison',
    name: 'Comparison',
    ...
    minAnswers: 2,
    maxAnswers: 20,
    helpText: 'Create pairs of items to compare (e.g., iPhone vs Samsung, Product A vs Product B)'
  }
];
...
export default function Step3QuestionsVoting({ data, updateData, onNext, onBack, electionTitle }) {
  ...
  const [selectedAuthMethod, setSelectedAuthMethod] = useState(data.auth_method || 'passkey');
  ...
  // ✅ MODIFIED: Add question with new structure
const addQuestion = (type) => {
  const newQuestion = {
    id: Date.now(),
    type: type, // Keep frontend type for UI
    question_text: '',
    question_type: 'multiple_choice', // ✅ ALWAYS use multiple_choice for ballot questions
    question_order: questions.length + 1,
    is_required: true,
    max_selections: 1,
    options: [
      { id: Date.now() + 1, option_text: '', option_order: 1 },
      { id: Date.now() + 2, option_text: '', option_order: 2 }
    ]
  };
  ...
  const deleteQuestion = (questionId) => {
    ...
    updateData({ questions: reorderedQuestions });
  ...
  // ✅ MODIFIED: Add option/candidate
  const addOption = (questionId) => {
    const updatedQuestions = questions.map(q => {
      if (q.id === questionId) {
        const newOptionOrder = q.options.length + 1;
        return {
          ...q,
          options: [
            ...q.options,
  ...
  // ✅ MODIFIED: Delete option
  const deleteOption = (questionId, optionId) => {
    const updatedQuestions = questions.map(q => {
      if (q.id === questionId) {
        const remainingOptions = q.options.filter(opt => opt.id !== optionId);
        // Reorder remaining options
        const reorderedOptions = remainingOptions.map((opt, idx) => ({
          ...opt,
          option_order: idx + 1
        }));
  ...
  const validateStep = () => {
    ...
    // ✅ MODIFIED: Validate questions and options
    questions.forEach((q) => {
      ...
      if (questionType && questionType.requiresAnswers) {
        const validOptions = q.options.filter(opt => opt.option_text && opt.option_text.trim());
        
        if (validOptions.length < questionType.minAnswers) {
          newErrors[`question_${q.id}_options`] = `At least ${questionType.minAnswers} options required`;
        }
      }
    });
  ...
  const handleContinue = () => {
    ...
    // ✅ IMPORTANT: Format questions for backend before sending
const formattedQuestions = questions.map(q => ({
  question_text: q.question_text,
  question_type: 'multiple_choice', // ✅ FORCE multiple_choice for all ballot questions
  question_order: q.question_order,
  is_required: q.is_required,
  max_selections: q.max_selections,
  options: q.options.filter(opt => opt.option_text.trim()).map(opt => ({
    ...
    updateData({ 
      ...
      voting_type: selectedVotingMethod,
  ...
  // ✅ CHANGED: Updated voting instructions with generic terms
  const getVotingInstructions = (questionType) => {
    if (!selectedVotingMethod) return '';
    
    const instructions = {
      plurality: {
        candidate_listing: 'Voters will select ONE option from the list',
        comparison: 'Voters will pick winners in each pair'
      },
      ranked_choice: {
        candidate_listing: 'Voters will RANK ALL options by preference (1st, 2nd, 3rd...)',
        comparison: 'Voters will RANK ALL items by preference'
      },
      approval: {
        candidate_listing: 'Voters will approve MULTIPLE options (Yes/No for each)',
        comparison: 'Voters will approve MULTIPLE items they support'
      }
    };
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6 border-2 border-indigo-200">
        ...
        <p className="text-gray-600">
          Configure authentication, voting method, and create ballot questions
        </p>
      </div>
      ...
      {/* ✅ MODIFIED: Add Question Buttons - Only 2 types now */}
      {selectedVotingMethod && (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {QUESTION_TYPES.map((type) => (
              ...
              >
                ...
                <p className="text-xs text-gray-500 italic">{type.helpText}</p>
              </div>
            ))}
          </div>
              ...
              <div key={question.id} className="bg-white rounded-xl shadow-lg p-6 border-2 border-gray-100">
                ...
                <div className="mb-6">
                  ...
                  <textarea
                    ...
                    placeholder={question.type === 'candidate_listing' 
                      ? "e.g., 'What is the best programming language?', 'Who should be President?', 'Vote for your favorite product'" 
                ...
                {/* ✅ CHANGED: Options Section - Generic terminology */}
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="block text-sm font-bold text-gray-700">
                      {question.type === 'candidate_listing' ? 'Choices/Options' : 'Comparison Items'} * 
                      (Min: {questionType?.minAnswers}, Max: {questionType?.maxAnswers})
                    </label>
                    {question.options.length < (questionType?.maxAnswers || 100) && (
                      <button
                    ...
                    {question.options.map((option, optionIndex) => (
                      <div key={option.id} className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          placeholder={question.type === 'candidate_listing' 
                            ? `Option ${optionIndex + 1} (e.g., JavaScript, Python, John Doe, Product A)` 
                            : `Item ${optionIndex + 1}`}
                        ...
                        {question.options.length > (questionType?.minAnswers || 2) && (
                          <button
                  ...
                  {errors[`question_${question.id}_options`] && (
                    <p className="text-red-500 text-sm mt-2">{errors[`question_${question.id}_options`]}</p>
                  )}
                </div>

                {/* Voting Method Info */}
                {selectedVotingMethod && (
                  <div className={`mt-4 p-4 bg-${questionType?.color}-50 border-2 border-${questionType?.color}-200 rounded-lg`}>
                    <p className="text-sm text-gray-700">
                      <FaInfoCircle className="inline mr-2" />
                      <strong>How voters will see this:</strong> {votingInstructions}
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl shadow-lg p-6 border-2 border-green-200">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                onClick={() => window.open(`https://api.whatsapp.com/send?text=Vote in my election!%20https://prod-client-omega.vercel.app/vote/${electionSlug}`, '_blank')}
...
//     details: 'Voters select ONE candidate from the ballot. Candidate with most votes wins.',
//     benefits: ['Simple and intuitive', 'Quick vote counting', 'Clear winners', 'Traditional democratic voting']
//   },
//   {
//     id: 'ranked_choice',
...
//     color: 'purple',
//     description: 'Preference ranking with elimination rounds',
//     details: 'Voters rank candidates by preference (1st, 2nd, 3rd...). Elimination rounds until majority.',
//     benefits: ['Eliminates spoiler effect', 'Ensures majority winner', 'More representative results', 'Encourages broader appeal']
//   },
//   {
//     id: 'approval',
...
//     details: 'Voters approve (Yes/No) multiple candidates they support.',
//     benefits: ['Reduces strategic voting', 'Simple ballot design', 'Encourages moderate candidates', 'Express broader preferences']
//   }
// ];

// // ✅ MODIFIED: Only 2 question types now - Candidate Listing and Comparison
// const QUESTION_TYPES = [
//   {
//     id: 'candidate_listing',
...
//     icon: <FaListOl />,
//     color: 'blue',
//     description: 'List of candidates/options for ballot-style voting',
//     requiresAnswers: true,
//     minAnswers: 2,
//     maxAnswers: 100,
//     helpText: 'Create a list of all candidates/options. Voters will see them as a ballot based on your voting type.'
//   },
//   {
//     id: 'comparison',
//     name: 'Comparison',
...
//   // ✅ MODIFIED: Get voting instructions based on question type
//   const getVotingInstructions = (questionType) => {
//     if (!selectedVotingMethod) return '';
    
//     const instructions = {
//       plurality: {
//         candidate_listing: 'Voters will select ONE candidate from the ballot',
//         comparison: 'Voters will pick winners in each pair'
//       },
//       ranked_choice: {
//         candidate_listing: 'Voters will RANK ALL candidates by preference (1st, 2nd, 3rd...)',
//         comparison: 'Voters will RANK ALL items by preference'
//       },
//       approval: {
//         candidate_listing: 'Voters will approve MULTIPLE candidates (Yes/No for each)',
//         comparison: 'Voters will approve MULTIPLE items they support'
//       }
//     };
...
//                     onChange={(e) => updateQuestion(question.id, 'question_text', e.target.value)}
//                     placeholder={question.type === 'candidate_listing' 
//                       ? "e.g., 'Who should be President?', 'Vote for your favorite product'" 
//                       : "e.g., 'Compare these two options'"}
//                     rows={3}
//                     className={`w-full px-4 py-3 border-2 rounded-xl focus:ring-2 focus:ring-${questionType?.color}-500 focus:border-${questionType?.color}-500 focus:outline-none resize-none text-base ${
//                       errors[`question_${question.id}_text`] ? 'border-red-500' : 'border-gray-300'
...
//                 {/* ✅ MODIFIED: Options/Candidates Section */}
//                 <div>
//                   <div className="flex items-center justify-between mb-3">
//                     <label className="block text-sm font-bold text-gray-700">
//                       {question.type === 'candidate_listing' ? 'Candidates/Options' : 'Comparison Items'} * 
//                       (Min: {questionType?.minAnswers}, Max: {questionType?.maxAnswers})
//                     </label>
//                     {question.options.length < (questionType?.maxAnswers || 100) && (
//                       <button
...
//                         className={`px-4 py-2 bg-${questionType?.color}-600 text-white rounded-lg font-semibold hover:bg-${questionType?.color}-700 transition-colors flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-${questionType?.color}-400`}
//                       >
//                         <FaPlus /> Add {question.type === 'candidate_listing' ? 'Candidate' : 'Item'}
//                       </button>
//                     )}
//                   </div>
...
//                           onChange={(e) => updateOption(question.id, option.id, e.target.value)}
//                           placeholder={question.type === 'candidate_listing' 
//                             ? `Candidate ${optionIndex + 1} name (e.g., John Doe, Product A)` 
//                             : `Item ${optionIndex + 1}`}
//                           className="flex-1 px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none text-base"
//                         />
...
//   );
// }
//last workable code, just to save answers to the database
// import React, { useState, useEffect } from 'react';
// import { toast } from 'react-toastify';
// import {
//   FaVoteYea,
...
//       type: type,
//       question_text: '',
//       question_type: type === 'candidate_listing' ? 'multiple_choice' : 'comparison', // Map to backend types
//       question_order: questions.length + 1,
//       is_required: true,
//       max_selections: 1,
//       options: [
...
//     const formattedQuestions = questions.map(q => ({
//       question_text: q.question_text,
//       question_type: q.question_type, // 'multiple_choice' or 'comparison'
//       question_order: q.question_order,
//       is_required: q.is_required,
//       max_selections: q.max_selections,
//       options: q.options.map(opt => ({
...
//   );
// }
//last workable perfect code,to remove open ended and image based above codes
// import React, { useState, useEffect } from 'react';
// import { toast } from 'react-toastify';
// import {
//   FaVoteYea,
