...
// KEY CHANGES:
// 1. Changed isEditMode to editMode to match parent component
// 2. Added support for displaying existing images from URLs (topic_image_url, logo_url)
// 3. Fixed preview logic to show both file uploads and existing URLs
// 4. Added proper handling for edit mode
...
// ✅ FIXED: Changed isEditMode to editMode to match parent component
export default function Step1BasicInfo({ 
  ...
  creatorType, 
  setElectionId, 
  editMode = false,  // ✅ Changed from isEditMode
  electionId         // ✅ Added electionId prop
}) {
  ...
  // ✅ FIXED: Initialize preview states
  const [previewImage, setPreviewImage] = useState(null);
  const [previewLogo, setPreviewLogo] = useState(null);

  // ✅ ADDED: Initialize previews from existing data (URLs or files)
  useEffect(() => {
    // Handle topic image preview
    if (data.topic_image instanceof File) {
      setPreviewImage(URL.createObjectURL(data.topic_image));
    } else if (data.topic_image_url) {
      setPreviewImage(data.topic_image_url);
    } else {
    ...
    if (data.logo instanceof File) {
      setPreviewLogo(URL.createObjectURL(data.logo));
    } else if (data.logo_url) {
      setPreviewLogo(data.logo_url);
    } else {
      setPreviewLogo(null);
    }
  }, [data.topic_image, data.topic_image_url, data.logo, data.logo_url]);
  ...
  useEffect(() => {
    // ✅ FIXED: Skip past date validation in edit mode for existing elections
    if (data.start_date && data.start_time) {
      const startDateTime = new Date(`${data.start_date}T${data.start_time}`);
      const now = new Date();
      
      // Only validate future date for NEW elections, not edits
      if (!editMode && startDateTime < now) {
  ...
  }, [data.start_date, data.start_time, data.end_date, data.end_time, editMode]);
  ...
  const validateStep = () => {
    ...
    if (data.start_date && data.end_date) {
      ...
      // ✅ FIXED: Only validate future date for NEW elections
      if (!editMode && startDateTime < now) {
    ...
    // ✅ Validate video watch requirements if enabled
  ...
  const handleImageChange = (e, type) => {
    ...
    if (type === 'topic_image') {
      // ✅ FIXED: Update both file and clear URL to indicate new upload
      updateData({ 
        topic_image: file,
        topic_image_url: null // Clear URL to indicate we're using a new file
      });
      ...
      updateData({ 
        logo: file,
        logo_url: null // Clear URL to indicate we're using a new file
      });
  ...
  const handleSaveAndContinue = async () => {
    ...
    try {
      // ✅ FIXED: Handle both create and edit modes
      if (editMode) {
        // In edit mode, just update local state and move to next step
        // The actual save happens on the final step (Step4Preview)
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border-2 border-blue-200">
        <h2 className="text-2xl font-bold text-gray-900 mb-2 flex items-center gap-2">
          ...
          {/* ✅ ADDED: Show edit mode indicator */}
          {editMode && (
        ...
        <p className="text-gray-600">
          {editMode 
            ? 'Update the fundamental details of your election'
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <label className="block text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
            ...
            {/* ✅ ADDED: Show indicator if image exists */}
            {previewImage && (
          ...
          {/* ✅ FIXED: Show preview for both file uploads AND existing URLs */}
          {previewImage && (
            <div className="mb-4 relative">
              ...
              <button
                type="button"
                onClick={() => handleRemoveImage('topic_image')}
                className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full hover:bg-red-600 transition-colors shadow-lg"
                title="Remove image"
              >
                ✕
              </button>
              {/* ✅ ADDED: Show source indicator */}
              <div className="absolute bottom-2 left-2">
                <span className={`px-2 py-1 text-xs font-semibold rounded ${
                  ...
                  {data.topic_image instanceof File ? '📁 New Upload' : '🔗 Existing'}
                </span>
              </div>
            </div>
          )}
          ...
          <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition-all">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="text-sm text-gray-600 font-semibold">
                {previewImage ? 'Click to replace image' : 'Click to upload image'}
              </p>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <label className="block text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
            ...
            Branding Logo
            {/* ✅ ADDED: Show indicator if logo exists */}
            {previewLogo && (
              <span className="ml-2 px-2 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full">
                ✓ Logo Set
              </span>
          ...
          <p className="text-sm text-gray-600 mb-4">
            {editMode && previewLogo 
              ? 'Current logo shown below. Upload a new one to replace it.'
            ...
            <div className="mb-4 relative">
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.src = 'https://via.placeholder.com/400x200?text=Logo+Not+Found';
                }}
              />
              <button
                type="button"
              ...
              <div className="absolute bottom-2 left-2">
                <span className={`px-2 py-1 text-xs font-semibold rounded ${
                  ...
                  {data.logo instanceof File ? '📁 New Upload' : '🔗 Existing'}
                </span>
              </div>
            </div>
          )}
          ...
          <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-purple-500 hover:bg-purple-50 transition-all">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              <FaUpload className="text-3xl text-gray-400 mb-2" />
              <p className="text-sm text-gray-600 font-semibold">
                {previewLogo ? 'Click to replace logo' : 'Click to upload logo'}
              </p>
              <p className="text-xs text-gray-500">PNG, JPG, WEBP (MAX 5MB)</p>
            </div>
            <input
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        <label className="block text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
          ...
          {/* ✅ ADDED: Show indicator if video exists */}
          {(data.topic_video_url || data.topic_video) && (
            <span className="ml-2 px-2 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full">
              ✓ Video Set
            </span>
          )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  // ✅ FIXED: Don't set min date in edit mode
      ...
      <div className="flex justify-end">
        ...
        >
          {loading ? (
            ...
            <>
              <FaCheckCircle />
              {editMode ? 'Update & Continue' : 'Save & Continue'}
            </>
          )}
        </button>
      </div>
...
//last workable code only to enable image and logo in edit mode above code
// // ✅ FIXED VERSION - Step1BasicInfo.jsx
// // KEY CHANGES:
