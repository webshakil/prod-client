...
import { publishElection, updateDraft, updateElection } from '../../../../../redux/api/election/electionApi';  // ✅ ADDED: updateElection import

// ✅ CHANGED: Added editMode prop
export default function Step4Preview({ data, onBack, onPublish, electionId, editMode = false }) {
  ...
  useEffect(() => {
    ...
    console.log('Edit Mode:', editMode);  // ✅ ADDED: Log edit mode
    console.log('Election ID:', electionId);
    console.log('Full data object:', data);
    console.log('Topic Image (File):', data?.topic_image);
    console.log('Topic Image URL:', data?.topic_image_url);
  ...
  // ✅ FIXED: Get lottery prize display - now correctly handles all reward types
  const getLotteryPrizeDisplay = () => {
    ...
    if (config.reward_type === 'non_monetary') {
      ...
      // ✅ FIXED: Show individual prizes if available
      let prizeDescription = config.prize_description || 'Prize package';
      if (config.non_monetary_prizes && config.non_monetary_prizes.length > 0) {
        prizeDescription = config.non_monetary_prizes.map(p => p.prize_description).join(', ');
      }
      
      return {
        type: 'Non-Monetary Prize',
        icon: '🎁',
        amount: `$${estimatedValue.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`,
        description: prizeDescription,
        value: estimatedValue
      };
    }
    
    // ✅ FIXED: Projected Revenue - show the projected_revenue amount, not calculated share
    if (config.reward_type === 'projected_revenue') {
      const projectedRevenue = parseFloat(config.projected_revenue || 0);
      const revenueSharePercentage = parseFloat(config.revenue_share_percentage || 0);
      
      // If revenue_share_percentage is set, calculate the prize pool from it
      // Otherwise, use projected_revenue as the total prize pool
      let prizePool = projectedRevenue;
      let description = `$${projectedRevenue.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} projected revenue prize pool`;
      
      if (revenueSharePercentage > 0) {
        prizePool = (projectedRevenue * revenueSharePercentage) / 100;
        description = `${revenueSharePercentage}% of $${projectedRevenue.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} projected revenue`;
      }
  ...
  // Handle publish button click - shows confirmation modal
  ...
  // ✅ ADDED: Handle Update Election (Edit Mode)
  const handleUpdateElection = async () => {
    setShowConfirmModal(false);
    console.log('✏️ User confirmed UPDATE (Edit Mode)');
    setPublishing(true);
    ...
    try {
      ...
      if (data.topic_image instanceof File) {
        files.topic_image = data.topic_image;
        console.log('📷 New topic image will be uploaded:', data.topic_image.name);
      }
      
      // ✅ Check if new video file was uploaded
      if (data.topic_video instanceof File) {
        files.topic_video = data.topic_video;
        console.log('🎥 New topic video will be uploaded:', data.topic_video.name);
      }
      
      // ✅ Check if new logo file was uploaded
      if (data.logo instanceof File) {
        files.logo = data.logo;
        console.log('🖼️ New logo will be uploaded:', data.logo.name);
      }

      console.log('📦 Update data prepared:', updateData);
      console.log('📁 Files to upload:', Object.keys(files));

      // Call updateElection API
      const response = await updateElection(electionId, updateData, files);
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to update election');
    } finally {
  ...
  const confirmPublish = async () => {
    // ✅ ADDED: If edit mode, call update function instead
    if (editMode) {
      return handleUpdateElection();
    }

    // Original publish logic for CREATE mode (unchanged)
    setShowConfirmModal(false);
    ...
    try {
      ...
      if (data.lottery_enabled && data.lottery_config) {
        ...
        if (data.lottery_config.reward_type === 'monetary') {
          ...
          // ✅ FIXED: Use projected_revenue as total_prize_pool when revenue_share_percentage is 0
          let calculatedPrizePool = lotteryConfig.projected_revenue;
          if (lotteryConfig.revenue_share_percentage > 0) {
            calculatedPrizePool = (lotteryConfig.projected_revenue * lotteryConfig.revenue_share_percentage) / 100;
          }
          ...
          if (lotteryConfig.revenue_share_percentage > 0) {
            ...
            lotteryConfig.prize_description = `$${lotteryConfig.projected_revenue.toLocaleString()} projected revenue prize pool`;
          }
  ...
  // ✅ ADDED: Helper function to get question options (supports both 'options' and 'answers' fields)
  const getQuestionOptions = (question) => {
    // Check for options array first (new format)
    if (question.options && Array.isArray(question.options) && question.options.length > 0) {
      return question.options.map(opt => opt.option_text || opt.text || opt);
    }
    // Fallback to answers array (old format)
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-6">
      {/* Header */}
      <div className={`${editMode ? 'bg-gradient-to-r from-orange-500 to-amber-500' : 'bg-gradient-to-r from-purple-600 to-pink-600'} rounded-2xl p-8 text-white shadow-2xl`}>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold mb-2 flex items-center gap-3">
              {editMode ? <><FaEdit /> Election Update Preview</> : <>📋 Election Preview</>}
            </h2>
            <p className={editMode ? 'text-orange-100' : 'text-purple-100'}>
              {editMode ? 'Review your changes before updating' : 'Review your election before publishing'}
            </p>
          </div>
      ...
      {editMode && (
        <div className="bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="font-semibold text-orange-900 mb-1">Edit Mode</h4>
              <p className="text-sm text-orange-800">
                You are editing an existing election. Some fields may not be editable if voting has already started.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        {expandedSections.basic && (
          <div className="p-6 space-y-4">
            ...
            {/* ✅ ADDED: Show image preview in Step 4 */}
            {(data?.topic_image instanceof File || data?.topic_image_url) && (
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="text-sm text-gray-600 mb-2">Cover Image</div>
                <img 
                  src={data.topic_image instanceof File ? URL.createObjectURL(data.topic_image) : data.topic_image_url}
                  alt="Cover"
                  className="h-32 object-cover rounded-lg"
                />
                {data.topic_image instanceof File && (
                  <span className="inline-block mt-2 px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded">
                    📁 New Upload: {data.topic_image.name}
                  </span>
                )}
              </div>
            )}

            {/* ✅ ADDED: Show logo preview in Step 4 */}
            {(data?.logo instanceof File || data?.logo_url) && (
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="text-sm text-gray-600 mb-2">Logo</div>
                <img 
                  src={data.logo instanceof File ? URL.createObjectURL(data.logo) : data.logo_url}
                  alt="Logo"
                  className="h-24 object-contain rounded-lg bg-white border"
                />
                {data.logo instanceof File && (
                  <span className="inline-block mt-2 px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded">
                    📁 New Upload: {data.logo.name}
                  </span>
                )}
              </div>
            )}
                   ...
                   data.lottery_config.prize_distribution.length > 0 && (
                    <div className="space-y-2">
                      ...
                      {data.lottery_config.prize_distribution.map((dist, idx) => {
                        ...
                        const amount = (prizeValue * (dist.percentage || 0)) / 100;
      ...
      {/* Questions Section - ✅ FIXED: Now shows options correctly */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <button
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500 mb-3">
                          Type: {question.question_type || question.type || 'Multiple Choice'} 
                          {(question.is_required || question.required) && ' • Required'}
                        </div>
                        
                        {/* ✅ FIXED: Display options correctly */}
                        {options.length > 0 ? (
                          <div className="space-y-2">
                            <div className="text-xs font-medium text-gray-600 mb-1">Options:</div>
                            {options.map((option, optIdx) => (
                              <div key={optIdx} className="flex items-center gap-2 text-sm text-gray-700 bg-white p-2 rounded border border-gray-200">
                                <div className="w-5 h-5 rounded-full border-2 border-pink-400 flex items-center justify-center flex-shrink-0">
                                  <span className="text-xs text-pink-500 font-bold">{optIdx + 1}</span>
                                </div>
                          ...
                          <div className="text-sm text-yellow-600 bg-yellow-50 p-2 rounded border border-yellow-200">
                            ⚠️ No options added for this question
                          </div>
                        )}
                      </div>
                    </div>
      ...
      <div className="flex justify-between items-center gap-4 pt-6">
        ...
        {/* ✅ CHANGED: Different button text/color for edit mode */}
        <button
          onClick={handlePublishClick}
          disabled={publishing}
          className={`px-10 py-4 ${editMode 
            ? 'bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600' 
            : 'bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700'
          } text-white rounded-xl font-bold text-lg transition-all transform hover:scale-105 shadow-xl flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed`}
        >
          {publishing ? (
            <>
            ...
            <>
              {editMode ? <><FaEdit /> Update Election</> : <><FaRocket /> Publish Election</>}
            </>
          )}
        </button>
      </div>
      ...
      {/* Info Box */}
      <div className={`${editMode ? 'bg-orange-50 border-orange-500' : 'bg-blue-50 border-blue-500'} border-l-4 p-6 rounded-lg`}>
        <div className="flex items-start gap-3">
          <FaExclamationTriangle className={editMode ? 'text-orange-600 mt-1' : 'text-blue-600 mt-1'} />
          <div>
            <h4 className={`font-semibold ${editMode ? 'text-orange-900' : 'text-blue-900'} mb-1`}>
              {editMode ? 'Before Updating' : 'Before Publishing'}
            </h4>
            <ul className={`text-sm ${editMode ? 'text-orange-800' : 'text-blue-800'} space-y-1`}>
              ...
              {editMode ? (
                <li>• Some changes may not be allowed if voting has started</li>
              ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 ${editMode ? 'bg-orange-100' : 'bg-green-100'} rounded-full flex items-center justify-center`}>
                  {editMode ? <FaEdit className="text-orange-600 text-xl" /> : <FaRocket className="text-green-600 text-xl" />}
                </div>
                <h3 className="text-xl font-bold text-gray-800">
                  {editMode ? 'Update Election?' : 'Publish Election?'}
                </h3>
              </div>
              <button
            ...
            <div className="mb-6">
              <p className="text-gray-600 mb-4">
                {editMode 
                  ? 'Your changes will be saved and the election will be updated.'
                  : 'Once published, your election will be live and voters can start participating.'
                }
              </p>
              <div className={`${editMode ? 'bg-orange-50 border-orange-200' : 'bg-blue-50 border-blue-200'} border rounded-lg p-4`}>
                <div className="flex items-start gap-2">
                  <FaExclamationTriangle className={`${editMode ? 'text-orange-600' : 'text-blue-600'} mt-0.5 flex-shrink-0`} />
                  <div className={`text-sm ${editMode ? 'text-orange-800' : 'text-blue-800'}`}>
                    <p className="font-semibold mb-1">Please note:</p>
                    <ul className="space-y-1">
                      {editMode ? (
                        <>
                          <li>• Changes will take effect immediately</li>
                          <li>• Some settings cannot be changed if voting has started</li>
                        </>
                      ) : (
                        <>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 ${editMode 
                  ? 'bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600' 
                  : 'bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700'
                } text-white rounded-xl font-semibold transition-colors flex items-center justify-center gap-2`}
              >
                {editMode ? <><FaEdit /> Yes, Update</> : <><FaRocket /> Yes, Publish</>}
              </button>
            </div>
          </div>
        </div>
...
//last workable code only to enable image and logo above code
// //last workable files
// // src/components/Dashboard/Tabs/election/ElectionWizard/Step4Preview.jsx
// import React, { useState, useEffect } from 'react';
// import { useSelector } from 'react-redux';
...
// //last workable files, only to show everything preview above code
// // src/components/Dashboard/Tabs/election/ElectionWizard/Step4Preview.jsx
// import React, { useState, useEffect } from 'react';
// import { useSelector } from 'react-redux';
// import { useNavigate } from 'react-router-dom';
