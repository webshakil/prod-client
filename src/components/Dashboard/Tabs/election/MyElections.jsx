...
// ✅ CHANGED: Import getAllElections instead of useGetMyElectionsQuery
import { getAllElections, deleteElection, cloneElection } from '../../../../redux/api/election/electionApi';
...
export default function MyElections() {
  ...
  // ✅ ADDED: State for elections, loading, and error (like VoteNow.jsx)
  const [elections, setElections] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  // ✅ ADDED: Get current user ID from localStorage
  ...
  // ✅ ADDED: Fetch elections using getAllElections (same as VoteNow.jsx)
  const fetchElections = async () => {
    try {
      ...
      console.log('📊 All Elections from API:', allElections);
      console.log('👤 Current User ID:', currentUserId);
      
      // ✅ Filter to only show elections created by the current user
      const myElections = allElections.filter(election => {
        const isOwner = String(election.creator_id) === String(currentUserId);
        console.log(`Election ${election.id}: creator_id=${election.creator_id}, currentUserId=${currentUserId}, isOwner=${isOwner}`);
        return isOwner;
      });
      
      console.log('✅ My Elections (filtered):', myElections);
      console.log('📊 First election vote_count:', myElections[0]?.vote_count);
  ...
  // ✅ ADDED: Fetch on component mount
  useEffect(() => {
    if (currentUserId) {
      fetchElections();
    } else {
  ...
  // ✅ Use elections from state instead of RTK Query
  const myElections = elections;
  ...
  // ✅ UPDATED: Calculate revenue using vote_count (same field as VoteNow.jsx)
  const calculateRevenue = (election) => {
    const votes = election.vote_count || election.total_votes || election.total_vote_count || 0;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {/* ✅ ADDED: Refresh button like VoteNow.jsx */}
        <button
          onClick={refetch}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          {/* ✅ UPDATED: Use vote_count like VoteNow.jsx */}
          <p className="text-3xl font-bold text-purple-600">
            {myElections.reduce((sum, e) => sum + (e.vote_count || e.total_votes || e.total_vote_count || 0), 0)}
          </p>
        </div>
      ...
      {filteredElections.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredElections.map((election) => {
            // ✅ UPDATED: Use vote_count like VoteNow.jsx
            const voteCount = election.vote_count || election.total_votes || election.total_vote_count || 0;
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {/* ✅ UPDATED: Now shows correct vote count */}
...
//last workbale code only to add vote count above code
// // src/components/Dashboard/Tabs/election/MyElections.jsx
