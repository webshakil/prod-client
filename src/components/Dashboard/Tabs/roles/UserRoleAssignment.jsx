...
export default function UserRoleAssignment() {
  ...
  const handleDelete = async (userId, roleName) => {
    // ✅ FIXED: Frontend protection - matches any role containing 'voter'
    if (roleName.toLowerCase().includes('voter')) {
    ...
    // ✅ ADDED: Frontend protection for Manager role
    if (roleName.toLowerCase() === 'manager') {
      toast.error('❌ Cannot delete Manager role - it is a protected system role', {
        position: 'top-center',
        autoClose: 4000,
      });
      return;
                            ...
                            {activeRoles.length > 0 && (
                              <div>
                                ...
                                <div className="space-y-2">
                                  {activeRoles.map((role, index) => {
                                    // ✅ FIXED: Check if role contains 'voter' (case-insensitive)
                                    const isVoter = role.role_name.toLowerCase().includes('voter');
                                    // ✅ ADDED: Check if role is 'manager' (case-insensitive)
                                    const isManager = role.role_name.toLowerCase() === 'manager';
                                    // ✅ Combined check for protected roles
                                    const isProtected = isVoter || isManager;
                                    ...
                                    return (
                                      <div key={index} className="flex items-center justify-between bg-white p-3 rounded-lg border border-gray-200">
                                        ...
                                        {/* ✅ NEVER show delete button for Voter or Manager roles */}
                                        {!isProtected && (
                                          <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAssignRole} className="p-6 space-y-4">
              ...
              {/* ✅ CHANGED: Assignment Type - Only Manual option now */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Assignment Type
                </label>
...
//last working code only to add option to not allowing delet manager above code
// //voter role will never be deleted
// import React, { useState, useMemo } from 'react';
// import { toast } from 'react-toastify';
// import { Search, UserPlus, X, Loader, Trash2, Calendar, User, Shield, ChevronDown, ChevronUp } from 'lucide-react';
...
//                                         {/* ✅ NEVER show delete button for any Voter role */}
//                                         {!isVoter && (
//                                           <button
//                                             onClick={() => handleDelete(user.user_id, role.role_name)}
//                                             disabled={deleting}
