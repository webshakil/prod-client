...
export default function Subscription() {
  ...
  // Fetch user's current subscription - ✅ ADD refetch
  const { data: subData, isLoading: subLoading, refetch } = useGetUserCurrentSubscriptionQuery();
  ...
  // ✅ NEW: Refetch subscription after payment success
  useEffect(() => {
    ...
    if (paymentStatus === 'success' && planId) {
      console.log(`💳 Payment success detected for plan ${planId} - refetching subscription data`);
      
      // Wait a moment for webhook to process, then refetch
      setTimeout(() => {
        refetch();
        console.log('✅ Subscription data refetched');
      }, 2000); // 2 second delay to let webhook complete
    }
  }, [refetch]);
