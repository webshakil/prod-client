...
//   // ✅ Get first question ID from election object (API requires it!)
//   const firstQuestionId = election?.questions?.[0]?.id;

//   // ✅ Pass BOTH electionId and questionId like CompactLiveResults does
//   const { data: results, isFetching } = useGetLiveResultsQuery(
//     { electionId: election?.id, questionId: firstQuestionId },
//     { 
//       skip: !election?.id || !firstQuestionId || !isOpen,
...
//   console.log('🎰 RealWinnerReveal - API Response:', { results, apiData, questions, totalVotes });
...
//   );
// }
// // ✅ FINAL WORKING VERSION - Uses correct votingApi endpoint
// import React, { useState, useEffect, useRef } from 'react';
// import { X, Trophy, RotateCcw, CheckCircle, Loader, AlertCircle } from 'lucide-react';
// import { useGetLiveResultsQuery } from '../../../../redux/api/voting/votingApi';
// //import { toast } from 'react-toastify';
// //import { useGetLiveResultsQuery } from '../../../redux/api/voting/votingApi';  // ✅ CORRECT API!
...
//   // ✅ CORRECT: Use same API as CompactLiveResults
//   const { data: results, isLoading: resultsLoading } = useGetLiveResultsQuery(
//     { electionId: election?.id },
//     { skip: !isOpen || !election?.id }
//   );

//   // ✅ Parse response same way as CompactLiveResults
//   const apiData = results?.data?.questions ? results.data : (results?.data || results);
//   const questions = apiData?.questions || [];
//   const totalVotes = apiData?.totalVotes || election?.total_vote_count || election?.vote_count || 0;
...
//   // AUTO-START
//   useEffect(() => {
//     if (isOpen && !hasStarted && !resultsLoading && lotteryWinners.length > 0) {
//       setHasStarted(true);
//       setTimeout(() => runLotteryReveal(), 2000);
//     }
//   }, [isOpen, hasStarted, resultsLoading, lotteryWinners]);
...
//   // Show modal with results + lottery
//   return (
//     <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
//       <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl" style={{ background: 'linear-gradient(180deg, #1f1f1f 0%, #0a0a0a 100%)', border: '4px solid #d4a418' }}>
//         <div className="flex justify-between items-center px-6 py-4 sticky top-0 z-10" style={{ background: 'linear-gradient(180deg, #d4a418 0%, #b8860b 100%)' }}>
...
//                 <div>
//                   <p className="text-yellow-300 font-bold text-lg">No Votes Recorded</p>
//                   <p className="text-yellow-400 text-sm">This election ended with 0 votes. Showing demo lottery.</p>
//                 </div>
//               </div>
//             </div>
//           ) : null}
