...
/*eslint-disable*/
  ...
  // ✅ NEW: Parse fee breakdown from transaction description
  const parseFeeBreakdown = (description) => {
    ...
    const earningsMatch = description.match(/Your earnings: \$([0-9.]+)/);
    ...
    return {
      ...
      gatewayType: stripeFeeMatch ? 'Stripe' : 'Paddle',
      platformFee: parseFloat(platformFeeMatch?.[1] || 0),
      yourEarnings: parseFloat(earningsMatch?.[1] || 0),
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ✅ NEW: Fee Information Banner */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        <Info className="text-blue-600 flex-shrink-0 mt-0.5" size={20} />
        <div className="text-sm text-blue-900">
          <p className="font-semibold mb-1">How earnings work:</p>
          <p>When voters pay to participate in your elections, we deduct processing fees (Stripe/Paddle) and platform fees (default 5%, varies by subscription). The remaining amount is <strong>frozen in your wallet</strong> until the election ends, then automatically released for withdrawal.</p>
        </div>
      </div>
            ...
            {transactionsData?.transactions?.map((transaction) => {
              ...
              return (
                <div key={transaction.id} className="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900">
                          {transaction.transaction_type === 'election_revenue' ? 'Election Revenue' : transaction.description}
                        </p>
                  ...
                  {breakdown && (
                    <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 text-sm">
                      <div className="grid grid-cols-2 gap-2">
                        <div className="text-gray-600">
                          <span className="font-medium">Voter Paid:</span> ${breakdown.voterPaid.toFixed(2)}
                        </div>
                        <div className="text-gray-600">
                          <span className="font-medium">{breakdown.gatewayType} Fee:</span> -${breakdown.gatewayFee.toFixed(2)}
                        </div>
                        <div className="text-gray-600">
                          <span className="font-medium">Platform Fee:</span> -${breakdown.platformFee.toFixed(2)}
                        </div>
                        <div className="text-green-600 font-semibold">
                          <span className="font-medium">You Received:</span> ${breakdown.yourEarnings.toFixed(2)}
                        </div>
                      </div>
                      {breakdown.frozenUntil && (
                        <p className="text-orange-600 text-xs mt-2">
                          🔒 Frozen until election ends: {breakdown.frozenUntil}
                        </p>
                      )}
                    </div>
                  )}
