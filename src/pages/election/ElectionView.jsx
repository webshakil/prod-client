...
export default function ElectionView() {
  ...
  // ✅ MODIFIED: VoteNowSection - Made more compact for side-by-side layout
  const VoteNowSection = () => {
    ...
    return (
      <div className={`rounded-xl shadow-lg p-5 ${colors.bg} border-2 ${colors.border} h-full flex flex-col`}>
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className={`text-lg font-bold ${colors.titleText} mb-1`}>
              {getStatusTitle()}
            </h3>
        ...
        <div className="mt-auto">
          <button
            ...
            className={`
              w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-base
        ...
        {/* Voting Eligibility Checklist */}
        <div className="mt-4 pt-3 border-t border-opacity-50" style={{ borderColor: 'inherit' }}>
          ...
          <div className="grid grid-cols-2 gap-1.5 text-xs">
            <div className={`flex items-center gap-1 ${!isOwner ? 'text-green-600' : 'text-red-500'}`}>
              {!isOwner ? <FaCheckCircle className="flex-shrink-0" /> : <FaTimesCircle className="flex-shrink-0" />}
              <span className="truncate">{isOwner ? 'Your election' : 'Can vote'}</span>
            </div>
            
            <div className={`flex items-center gap-1 ${(election.status === 'published' || election.status === 'active') ? 'text-green-600' : 'text-red-500'}`}>
              {(election.status === 'published' || election.status === 'active') ? <FaCheckCircle className="flex-shrink-0" /> : <FaTimesCircle className="flex-shrink-0" />}
              <span className="truncate">{(election.status === 'published' || election.status === 'active') ? 'Published' : election.status}</span>
            </div>
            ...
            {(() => {
              ...
              return (
                <div className={`flex items-center gap-1 ${hasStarted ? 'text-green-600' : 'text-yellow-600'}`}>
                  {hasStarted ? <FaCheckCircle className="flex-shrink-0" /> : <FaClock className="flex-shrink-0" />}
                  <span className="truncate">{hasStarted ? 'Started' : 'Not started'}</span>
                </div>
              );
            })()}
            ...
            {(() => {
              ...
              return (
                <div className={`flex items-center gap-1 ${!hasEnded ? 'text-green-600' : 'text-red-500'}`}>
                  {!hasEnded ? <FaCheckCircle className="flex-shrink-0" /> : <FaTimesCircle className="flex-shrink-0" />}
                  <span className="truncate">{hasEnded ? 'Ended' : 'Not ended'}</span>
                </div>
              );
            })()}
          </div>
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          ...
          {/* ✅ MODIFIED: Vote Now + Lottery Slot Machine Side by Side */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            {/* Vote Now Section */}
            <VoteNowSection />

            {/* Lottery Slot Machine Section */}
            {(() => {
...
//last working code only to keep slot machine and vote now side by side above code
// import React, { useState, useEffect } from 'react';
// import { useParams, useNavigate, useLocation } from 'react-router-dom';
// import { useDispatch } from 'react-redux';
// import { toast } from 'react-toastify';
...
//             <div className={`flex items-center gap-1 ${!isOwner ? 'text-green-600' : 'text-red-500'}`}>
//               {!isOwner ? <FaCheckCircle /> : <FaTimesCircle />}
//               <span>Not your election</span>
//             </div>
//             <div className={`flex items-center gap-1 ${election.status !== 'draft' ? 'text-green-600' : 'text-red-500'}`}>
//               {election.status !== 'draft' ? <FaCheckCircle /> : <FaTimesCircle />}
//               <span>Published</span>
//             </div>
//             <div className={`flex items-center gap-1 ${new Date() >= new Date(election.start_date) ? 'text-green-600' : 'text-yellow-600'}`}>
//               {new Date() >= new Date(election.start_date) ? <FaCheckCircle /> : <FaClock />}
//               <span>Started</span>
//             </div>
//             <div className={`flex items-center gap-1 ${new Date() <= new Date(election.end_date) ? 'text-green-600' : 'text-red-500'}`}>
//               {new Date() <= new Date(election.end_date) ? <FaCheckCircle /> : <FaTimesCircle />}
//               <span>Not ended</span>
//             </div>
//           </div>
//         </div>
...
//               {/* Debug: Log lottery data to console */}
//               {console.log('🎰 Lottery Debug:', {
//                 lottery_enabled: election.lottery_enabled,
//                 lottery_config: election.lottery_config,
//                 lottery_prize_distribution: election.lottery_prize_distribution,
//                 prize_distribution_from_config: election.lottery_config?.prize_distribution,
//                 full_election: election
//               })}
...
//                           // Debug: Log prize object to see actual field names
...
//                           // Extract prize value from multiple possible field names
//                           const getPrizeValue = (prizeObj) => {
//                             const possibleFields = [
//                               'prize_value', 'prizeValue', 'amount', 'value', 
//                               'reward_amount', 'rewardAmount', 'prize_amount', 
//                               'prizeAmount', 'reward', 'prize', 'money'
//                             ];
                            
//                             for (const field of possibleFields) {
//                               if (prizeObj[field] !== undefined && prizeObj[field] !== null) {
//                                 return parseFloat(prizeObj[field]);
//                               }
//                             }
//                             return 0;
//                           };
...
//                                     {getRankLabel(prize.rank || idx + 1)}
//                                   </p>
//                                   {(prize.prize_description || prize.description) && (
//                                     <p className="text-sm text-gray-600">{prize.prize_description || prize.description}</p>
//                                   )}
//                                 </div>
//                               </div>
//                               <div className="text-right">
...
//   );
// }
//last workable code only to add vote now and gamification solution abvoe code
// import React, { useState, useEffect } from 'react';
// import { useParams, useNavigate, useLocation } from 'react-router-dom';
// import { useDispatch } from 'react-redux';
// import { toast } from 'react-toastify';
