...
// Added CSV and PDF Download functionality
...
import {
  ...
  MapPin, Percent, ArrowUpRight, Wallet, Download
} from 'lucide-react';
import {
...
export default function LiveAnalytics() {
  ...
  const downloadPDF = (title, content, filename) => {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text(String(title || 'Report'), 14, 20);
      ...
      if (!content || content.length === 0) {
        ...
        console.log('Processing', content.length, 'content sections');
        ...
        content.forEach((section, index) => {
          ...
          if (section.type === 'heading') {
            ...
            doc.text(String(section.text || ''), 14, yPos);
            yPos += 12;
            ...
            console.log('Drawing table with', section.data.length, 'rows');
            try {
              if (typeof autoTable === 'function') {
                ...
                console.log('Table drawn, new yPos:', yPos);
              } else {
                // Fallback: render table as text
                console.log('autoTable not available, using text fallback');
                doc.text('Table: ' + (section.headers?.join(', ') || 'Data'), 14, yPos);
                yPos += 10;
              }
            } catch (tableError) {
              console.error('Error drawing table:', tableError);
              yPos += 10;
            }
  ...
  // ========== Section-Specific Download Handlers ==========
  ...
  const handleDownloadAllCSV = () => {
    ...
    if (report?.overview) {
      allData.push({
        Section: 'Overview',
        Metric: 'Total Users',
        Value: report.overview.total_users || 0
      });
      allData.push({
        Section: 'Overview',
        Metric: 'Total Elections',
        Value: report.overview.total_elections || 0
      });
      allData.push({
        Section: 'Overview',
        Metric: 'Total Votes',
        Value: report.overview.total_votes || 0
      });
      allData.push({
        Section: 'Overview',
        Metric: 'Active Subscriptions',
        Value: report.overview.active_subscriptions || 0
      });
      allData.push({
        Section: 'Overview',
        Metric: 'Lottery Elections',
        Value: report.overview.lottery_elections || 0
      });
      allData.push({
        Section: 'Overview',
        Metric: 'Total Prize Pool',
        Value: report.overview.total_prize_pool || 0
      });
    }
    
    // Users by Country
    if (report?.users?.byCountry) {
      report.users.byCountry.forEach(item => {
        allData.push({
          Section: 'Users by Country',
          Metric: item.country || 'Unknown',
          Value: item.count
        });
      });
    }
    ...
    // Users by Gender
    if (report?.users?.byGender) {
      report.users.byGender.forEach(item => {
        allData.push({
          Section: 'Users by Gender',
          Metric: item.gender || 'Not Specified',
          Value: item.count
        });
      });
    }
    ...
    // Users by Age
    if (report?.users?.byAge) {
      report.users.byAge.forEach(item => {
        allData.push({
          Section: 'Users by Age',
          Metric: item.age_group,
          Value: item.count
        });
      });
    }
    
    downloadCSV(allData, 'platform_analytics_full');
  };
  ...
  const handleDownloadAllPDF = () => {
    ...
    // Overview Section
    content.push({ type: 'heading', text: 'Platform Overview' });
    if (report?.overview) {
      content.push({
        type: 'stats',
        items: [
          { label: 'Total Users', value: formatNumber(report.overview.total_users) },
          { label: 'Total Elections', value: formatNumber(report.overview.total_elections) },
          { label: 'Total Votes', value: formatNumber(report.overview.total_votes) },
          { label: 'Active Subscriptions', value: formatNumber(report.overview.active_subscriptions) },
          { label: 'Lottery Elections', value: formatNumber(report.overview.lottery_elections) },
          { label: 'Total Prize Pool', value: formatCurrency(report.overview.total_prize_pool) },
        ]
      });
    }
    ...
    // Real-time Stats
    if (realtime?.stats) {
      content.push({ type: 'heading', text: 'Real-time Statistics' });
      content.push({
        type: 'stats',
        items: [
          { label: 'Active Elections', value: realtime.stats.active_elections || 0 },
          { label: 'Published Elections', value: realtime.stats.published_elections || 0 },
          { label: 'Votes (Last Hour)', value: realtime.stats.votes_last_hour || 0 },
          { label: 'Votes (Last 24h)', value: realtime.stats.votes_last_24h || 0 },
          { label: 'New Users (24h)', value: realtime.stats.new_users_24h || 0 },
          { label: 'Pending Lottery Draws', value: realtime.stats.pending_lottery_draws || 0 },
        ]
      });
    }
    
    // User Statistics
    if (report?.users?.stats) {
      content.push({ type: 'heading', text: 'User Statistics' });
      content.push({
        type: 'stats',
        items: [
          { label: 'Total Registered', value: formatNumber(report.users.stats.total_registered) },
          { label: `New Users (${period}d)`, value: formatNumber(report.users.stats.new_users_period) },
          { label: 'New This Week', value: formatNumber(report.users.stats.new_users_week) },
          { label: 'New Today', value: formatNumber(report.users.stats.new_users_today) },
        ]
      });
    }
    
    // Users by Country Table
    if (report?.users?.byCountry?.length > 0) {
      content.push({ type: 'heading', text: 'Users by Country' });
      content.push({
        type: 'table',
        headers: ['Country', 'Count', 'Percentage'],
        data: report.users.byCountry.map(item => {
          const total = report.users.byCountry.reduce((a, b) => a + parseInt(b.count), 0);
          return [item.country || 'Unknown', item.count, ((parseInt(item.count) / total) * 100).toFixed(1) + '%'];
        })
      });
    }
    
    // Vote Statistics
    if (report?.votes?.stats) {
      content.push({ type: 'heading', text: 'Vote Statistics' });
      content.push({
        type: 'stats',
        items: [
          { label: 'Total Votes', value: formatNumber(report.votes.stats.total_votes) },
          { label: 'Valid Votes', value: formatNumber(report.votes.stats.valid_votes) },
          { label: 'Unique Voters', value: formatNumber(report.votes.stats.unique_voters) },
          { label: `Votes (${period}d)`, value: formatNumber(report.votes.stats.votes_period) },
          { label: 'Votes This Week', value: formatNumber(report.votes.stats.votes_week) },
          { label: 'Votes Today', value: formatNumber(report.votes.stats.votes_today) },
        ]
      });
    }
    
    // Lottery Statistics
    if (report?.lottery?.stats) {
      content.push({ type: 'heading', text: 'Lottery Statistics' });
      content.push({
        type: 'stats',
        items: [
          { label: 'Total Draws', value: formatNumber(report.lottery.stats.total_draws) },
          { label: 'Completed Draws', value: formatNumber(report.lottery.stats.completed_draws) },
          { label: 'Total Winners', value: formatNumber(report.lottery.stats.total_winner_slots) },
        ]
      });
    }
    
    // Top Elections Table
    if (report?.elections?.topElections?.length > 0) {
      content.push({ type: 'heading', text: 'Top Elections' });
      content.push({
        type: 'table',
        headers: ['Title', 'Status', 'Votes', 'Views'],
        data: report.elections.topElections.map(e => [
          e.title?.substring(0, 40) || 'N/A',
          e.status || 'N/A',
          e.vote_count || 0,
          e.view_count || 0
        ])
    ...
    downloadPDF('Platform Analytics Report', content, 'platform_analytics_full');
  };

  // Revenue Section Downloads
  const handleDownloadRevenueCSV = () => {
    const data = [];
    
    data.push({
      Category: 'Estimated Monthly Subscription',
      Value: estimatedSubscriptionRevenue,
      Details: `${activeSubscriptions} active subscribers`
    });
    data.push({
      Category: 'Total Lottery Prize Pool',
      Value: totalParticipationFees,
      Details: `${report?.overview?.lottery_elections || 0} lottery elections`
    });
    data.push({
      Category: 'Total Usage Revenue',
      Value: totalUsageAmount,
      Details: 'Pay-as-you-go earnings'
    });
    data.push({
      Category: 'Paid Elections',
      Value: paidElections,
      Details: `${freeElections} free elections`
    });
    
    if (report?.subscriptions?.stats) {
      data.push({ Category: 'Active Subscriptions', Value: report.subscriptions.stats.active || 0, Details: '' });
      data.push({ Category: 'Recurring Plans', Value: report.subscriptions.stats.recurring_count || 0, Details: '' });
      data.push({ Category: 'Pay-as-you-go', Value: report.subscriptions.stats.pay_as_you_go_count || 0, Details: '' });
      data.push({ Category: 'Auto-renew Enabled', Value: report.subscriptions.stats.auto_renew_enabled || 0, Details: '' });
    }
    
    if (report?.subscriptions?.byGateway) {
      report.subscriptions.byGateway.forEach(g => {
        data.push({ Category: `Gateway - ${g.gateway}`, Value: g.count, Details: '' });
      });
    }
    
    downloadCSV(data, 'revenue_report');
  };

  const handleDownloadRevenuePDF = () => {
    const content = [];
    
    content.push({ type: 'heading', text: 'Revenue Overview' });
    content.push({
      type: 'stats',
      items: [
        { label: 'Est. Monthly Subscription', value: formatCurrency(estimatedSubscriptionRevenue) },
        { label: 'Total Lottery Prize Pool', value: formatCurrency(totalParticipationFees) },
        { label: 'Total Usage Revenue', value: formatCurrency(totalUsageAmount) },
        { label: 'Paid Elections', value: paidElections },
        { label: 'Free Elections', value: freeElections },
      ]
    });
    
    if (report?.subscriptions?.stats) {
      content.push({ type: 'heading', text: 'Subscription Breakdown' });
      content.push({
        type: 'stats',
        items: [
          { label: 'Active Subscriptions', value: formatNumber(report.subscriptions.stats.active) },
          { label: 'Recurring Plans', value: formatNumber(report.subscriptions.stats.recurring_count) },
          { label: 'Pay-as-you-go', value: formatNumber(report.subscriptions.stats.pay_as_you_go_count) },
          { label: 'Auto-renew Enabled', value: formatNumber(report.subscriptions.stats.auto_renew_enabled) },
        ]
      });
    }
    
    if (report?.subscriptions?.byGateway?.length > 0) {
      content.push({ type: 'heading', text: 'Payment Gateway Distribution' });
      content.push({
        type: 'table',
        headers: ['Gateway', 'Count'],
        data: report.subscriptions.byGateway.map(g => [g.gateway, g.count])
      });
    }
    
    downloadPDF('Platform Revenue Report', content, 'revenue_report');
  };

  // Platform Revenue Section Downloads
  const handleDownloadPlatformRevenueCSV = () => {
    if (!platformRevenue) {
      alert('No platform revenue data available');
      return;
    }
    ...
    // Summary
    if (platformRevenue.summary) {
      data.push({ Category: 'Summary', Metric: 'Total Platform Revenue', Value: platformRevenue.summary.total_platform_revenue || 0 });
      data.push({ Category: 'Summary', Metric: 'Revenue This Period', Value: platformRevenue.summary.revenue_this_period || 0 });
      data.push({ Category: 'Summary', Metric: 'Total Subscription Revenue', Value: platformRevenue.summary.total_subscription_revenue || 0 });
      data.push({ Category: 'Summary', Metric: 'Subscription Revenue Period', Value: platformRevenue.summary.subscription_revenue_period || 0 });
      data.push({ Category: 'Summary', Metric: 'Total Platform Fee Revenue', Value: platformRevenue.summary.total_platform_fee_revenue || 0 });
      data.push({ Category: 'Summary', Metric: 'Platform Fee Revenue Period', Value: platformRevenue.summary.platform_fee_revenue_period || 0 });
    }
    
    // Subscription Stats
    if (platformRevenue.subscriptions?.stats) {
      data.push({ Category: 'Subscriptions', Metric: 'Active Subscriptions', Value: platformRevenue.subscriptions.stats.active_subscriptions || 0 });
      data.push({ Category: 'Subscriptions', Metric: 'Total Subscriptions', Value: platformRevenue.subscriptions.stats.total_subscriptions || 0 });
      data.push({ Category: 'Subscriptions', Metric: 'Revenue (30d)', Value: platformRevenue.subscriptions.stats.revenue_last_30_days || 0 });
      data.push({ Category: 'Subscriptions', Metric: 'Revenue (7d)', Value: platformRevenue.subscriptions.stats.revenue_last_7_days || 0 });
      data.push({ Category: 'Subscriptions', Metric: 'Revenue (Today)', Value: platformRevenue.subscriptions.stats.revenue_today || 0 });
    }
    
    // Platform Fee Stats
    if (platformRevenue.platformFees?.stats) {
      data.push({ Category: 'Platform Fees', Metric: 'Successful Payments', Value: platformRevenue.platformFees.stats.successful_payments || 0 });
      data.push({ Category: 'Platform Fees', Metric: 'Pending Payments', Value: platformRevenue.platformFees.stats.pending_payments || 0 });
      data.push({ Category: 'Platform Fees', Metric: 'Platform Fee (30d)', Value: platformRevenue.platformFees.stats.platform_fee_last_30_days || 0 });
      data.push({ Category: 'Platform Fees', Metric: 'Platform Fee (7d)', Value: platformRevenue.platformFees.stats.platform_fee_last_7_days || 0 });
      data.push({ Category: 'Platform Fees', Metric: 'Platform Fee (Today)', Value: platformRevenue.platformFees.stats.platform_fee_today || 0 });
    }
    
    // By Gateway
    if (platformRevenue.platformFees?.byGateway) {
      platformRevenue.platformFees.byGateway.forEach(g => {
        data.push({ Category: 'Gateway', Metric: g.gateway, Value: g.total_platform_fee, PaymentCount: g.payment_count, TotalAmount: g.total_amount });
      });
    }
    
    // Top Elections
    if (platformRevenue.platformFees?.topElections) {
      /*eslint-disable*/
      platformRevenue.platformFees.topElections.forEach((e, idx) => {
        data.push({ 
          Category: 'Top Elections', 
          Metric: e.election_title || `Election #${e.election_id}`,
          Value: e.total_platform_fee,
          PaymentCount: e.payment_count,
          TotalAmount: e.total_amount
        });
      });
    }
    
    downloadCSV(data, 'platform_revenue_report');
  };

  const handleDownloadPlatformRevenuePDF = () => {
    if (!platformRevenue) {
      alert('No platform revenue data available');
      return;
    }
    ...
    content.push({ type: 'heading', text: 'Platform Revenue Summary' });
    if (platformRevenue.summary) {
      content.push({
        type: 'stats',
        items: [
          { label: 'Total Platform Revenue', value: formatCurrency(platformRevenue.summary.total_platform_revenue) },
          { label: `Revenue This Period (${period}d)`, value: formatCurrency(platformRevenue.summary.revenue_this_period) },
          { label: 'Total Subscription Revenue', value: formatCurrency(platformRevenue.summary.total_subscription_revenue) },
          { label: 'Total Platform Fee Revenue', value: formatCurrency(platformRevenue.summary.total_platform_fee_revenue) },
        ]
      });
    }
    
    content.push({ type: 'heading', text: 'Subscription Details' });
    if (platformRevenue.subscriptions?.stats) {
      content.push({
        type: 'stats',
        items: [
          { label: 'Active Subscriptions', value: formatNumber(platformRevenue.subscriptions.stats.active_subscriptions) },
          { label: 'Total Subscriptions', value: formatNumber(platformRevenue.subscriptions.stats.total_subscriptions) },
          { label: 'Revenue (30d)', value: formatCurrency(platformRevenue.subscriptions.stats.revenue_last_30_days) },
          { label: 'Revenue (7d)', value: formatCurrency(platformRevenue.subscriptions.stats.revenue_last_7_days) },
          { label: 'Revenue (Today)', value: formatCurrency(platformRevenue.subscriptions.stats.revenue_today) },
        ]
      });
    }
    
    content.push({ type: 'heading', text: 'Platform Fee Details' });
    if (platformRevenue.platformFees?.stats) {
      content.push({
        type: 'stats',
        items: [
          { label: 'Successful Payments', value: formatNumber(platformRevenue.platformFees.stats.successful_payments) },
          { label: 'Pending Payments', value: formatNumber(platformRevenue.platformFees.stats.pending_payments) },
          { label: 'Platform Fee (30d)', value: formatCurrency(platformRevenue.platformFees.stats.platform_fee_last_30_days) },
          { label: 'Platform Fee (7d)', value: formatCurrency(platformRevenue.platformFees.stats.platform_fee_last_7_days) },
          { label: 'Platform Fee (Today)', value: formatCurrency(platformRevenue.platformFees.stats.platform_fee_today) },
        ]
      });
    }
    
    if (platformRevenue.platformFees?.byGateway?.length > 0) {
      content.push({ type: 'heading', text: 'Revenue by Payment Gateway' });
      content.push({
        type: 'table',
        headers: ['Gateway', 'Payments', 'Total Amount', 'Platform Fee'],
        data: platformRevenue.platformFees.byGateway.map(g => [
          g.gateway || 'Unknown',
          g.payment_count,
          formatCurrency(g.total_amount),
          formatCurrency(g.total_platform_fee)
        ])
      });
    }
    
    if (platformRevenue.platformFees?.topElections?.length > 0) {
      content.push({ type: 'heading', text: 'Top Elections by Platform Fee' });
      content.push({
        type: 'table',
        headers: ['Election', 'Payments', 'Total Amount', 'Platform Fee'],
        data: platformRevenue.platformFees.topElections.map(e => [
          (e.election_title || `Election #${e.election_id}`).substring(0, 30),
          e.payment_count,
          formatCurrency(e.total_amount),
          formatCurrency(e.total_platform_fee)
        ])
      });
    }
    
    downloadPDF('Platform Revenue Report', content, 'platform_revenue_report');
  };

  // User Analytics Downloads
  const handleDownloadUsersCSV = () => {
    const data = [];
    
    // Stats
    if (report?.users?.stats) {
      data.push({ Category: 'Stats', Item: 'Total Registered', Value: report.users.stats.total_registered || 0 });
      data.push({ Category: 'Stats', Item: `New (${period}d)`, Value: report.users.stats.new_users_period || 0 });
      data.push({ Category: 'Stats', Item: 'New This Week', Value: report.users.stats.new_users_week || 0 });
      data.push({ Category: 'Stats', Item: 'New Today', Value: report.users.stats.new_users_today || 0 });
    }
    
    // By Country
    if (report?.users?.byCountry) {
      report.users.byCountry.forEach(item => {
        data.push({ Category: 'By Country', Item: item.country || 'Unknown', Value: item.count });
      });
    }
    ...
    if (report?.users?.byGender) {
      report.users.byGender.forEach(item => {
        data.push({ Category: 'By Gender', Item: item.gender || 'Not Specified', Value: item.count });
      });
    }
    ...
    if (report?.users?.byAge) {
      report.users.byAge.forEach(item => {
        data.push({ Category: 'By Age', Item: item.age_group, Value: item.count });
      });
    }
    
    // Trend
    if (report?.users?.trend) {
      report.users.trend.forEach(item => {
        data.push({ Category: 'Registration Trend', Item: item.date, Value: item.count });
      });
    }
    
    downloadCSV(data, 'user_analytics');
  };
    ...
    if (report?.users?.byCountry?.length > 0) {
      ...
      content.push({
        ...
        data: report.users.byCountry.map(item => [
          ...
          item.count,
          ((parseInt(item.count) / total) * 100).toFixed(1) + '%'
        ])
      });
    }
    
    if (report?.users?.byGender?.length > 0) {
      content.push({ type: 'heading', text: 'Users by Gender' });
      content.push({
        type: 'table',
        headers: ['Gender', 'Count'],
        data: report.users.byGender.map(item => [item.gender || 'Not Specified', item.count])
      });
    }
    
    if (report?.users?.byAge?.length > 0) {
      content.push({ type: 'heading', text: 'Users by Age Group' });
      content.push({
        type: 'table',
        headers: ['Age Group', 'Count'],
        data: report.users.byAge.map(item => [item.age_group, item.count])
      });
    }
    
    downloadPDF('User Analytics Report', content, 'user_analytics');
  };

  // Elections Downloads
  const handleDownloadElectionsCSV = () => {
    const data = [];
    
    // Stats
    if (report?.elections?.stats) {
      data.push({ Category: 'Status', Item: 'Draft', Value: report.elections.stats.draft || 0 });
      data.push({ Category: 'Status', Item: 'Published', Value: report.elections.stats.published || 0 });
      data.push({ Category: 'Status', Item: 'Active', Value: report.elections.stats.active || 0 });
      data.push({ Category: 'Status', Item: 'Completed', Value: report.elections.stats.completed || 0 });
      data.push({ Category: 'Status', Item: 'Cancelled', Value: report.elections.stats.cancelled || 0 });
      data.push({ Category: 'Voting Type', Item: 'Plurality', Value: report.elections.stats.plurality || 0 });
      data.push({ Category: 'Voting Type', Item: 'Ranked Choice', Value: report.elections.stats.ranked_choice || 0 });
      data.push({ Category: 'Voting Type', Item: 'Approval', Value: report.elections.stats.approval || 0 });
      data.push({ Category: 'Features', Item: 'Lottery Enabled', Value: report.elections.stats.lottery_enabled_count || 0 });
      data.push({ Category: 'Features', Item: 'Biometric Required', Value: report.elections.stats.biometric_required_count || 0 });
      data.push({ Category: 'Features', Item: 'Anonymous Voting', Value: report.elections.stats.anonymous_voting_count || 0 });
      data.push({ Category: 'Features', Item: 'Video Required', Value: report.elections.stats.video_required_count || 0 });
      data.push({ Category: 'Features', Item: 'Live Results', Value: report.elections.stats.live_results_count || 0 });
      data.push({ Category: 'Features', Item: 'Vote Editing', Value: report.elections.stats.vote_editing_count || 0 });
    }
    
    // Top Elections
    if (report?.elections?.topElections) {
      report.elections.topElections.forEach(e => {
        data.push({ 
          Category: 'Top Elections', 
          Item: e.title, 
          Value: e.vote_count || 0,
          Status: e.status,
          Views: e.view_count || 0,
          Lottery: e.lottery_enabled ? 'Yes' : 'No',
          Free: e.is_free ? 'Yes' : 'No'
        });
      });
    }
    
    downloadCSV(data, 'election_analytics');
  };

  const handleDownloadElectionsPDF = () => {
    const content = [];
    
    content.push({ type: 'heading', text: 'Election Status Distribution' });
    if (report?.elections?.stats) {
      content.push({
        type: 'stats',
        items: [
          { label: 'Draft', value: report.elections.stats.draft || 0 },
          { label: 'Published', value: report.elections.stats.published || 0 },
          { label: 'Active', value: report.elections.stats.active || 0 },
          { label: 'Completed', value: report.elections.stats.completed || 0 },
          { label: 'Cancelled', value: report.elections.stats.cancelled || 0 },
        ]
      });
    }
    
    content.push({ type: 'heading', text: 'Voting Methods' });
    if (report?.elections?.stats) {
      content.push({
        type: 'stats',
        items: [
          { label: 'Plurality', value: report.elections.stats.plurality || 0 },
          { label: 'Ranked Choice', value: report.elections.stats.ranked_choice || 0 },
          { label: 'Approval', value: report.elections.stats.approval || 0 },
        ]
      });
    }
    
    content.push({ type: 'heading', text: 'Feature Usage' });
    if (report?.elections?.stats) {
      content.push({
        type: 'stats',
        items: [
          { label: 'Lottery Enabled', value: report.elections.stats.lottery_enabled_count || 0 },
          { label: 'Biometric Required', value: report.elections.stats.biometric_required_count || 0 },
          { label: 'Anonymous Voting', value: report.elections.stats.anonymous_voting_count || 0 },
          { label: 'Video Required', value: report.elections.stats.video_required_count || 0 },
          { label: 'Live Results', value: report.elections.stats.live_results_count || 0 },
          { label: 'Vote Editing', value: report.elections.stats.vote_editing_count || 0 },
        ]
      });
    }
    ...
    if (report?.elections?.topElections?.length > 0) {
      ...
      content.push({
        type: 'table',
        headers: ['Title', 'Status', 'Votes', 'Views', 'Lottery', 'Free'],
        data: report.elections.topElections.map(e => [
          (e.title || 'N/A').substring(0, 25),
          e.status || 'N/A',
          e.vote_count || 0,
          e.view_count || 0,
          e.lottery_enabled ? 'Yes' : 'No',
          e.is_free ? 'Yes' : 'No'
        ])
      });
    }
    
    downloadPDF('Election Analytics Report', content, 'election_analytics');
  };

  // Votes & Lottery Downloads
  const handleDownloadVotesCSV = () => {
    const data = [];
    
    if (report?.votes?.stats) {
      data.push({ Category: 'Votes', Metric: 'Total Votes', Value: report.votes.stats.total_votes || 0 });
      data.push({ Category: 'Votes', Metric: 'Valid Votes', Value: report.votes.stats.valid_votes || 0 });
      data.push({ Category: 'Votes', Metric: 'Unique Voters', Value: report.votes.stats.unique_voters || 0 });
      data.push({ Category: 'Votes', Metric: `Votes (${period}d)`, Value: report.votes.stats.votes_period || 0 });
      data.push({ Category: 'Votes', Metric: 'Votes This Week', Value: report.votes.stats.votes_week || 0 });
      data.push({ Category: 'Votes', Metric: 'Votes Today', Value: report.votes.stats.votes_today || 0 });
    }
    
    if (report?.lottery?.stats) {
      data.push({ Category: 'Lottery', Metric: 'Total Draws', Value: report.lottery.stats.total_draws || 0 });
      data.push({ Category: 'Lottery', Metric: 'Completed Draws', Value: report.lottery.stats.completed_draws || 0 });
      data.push({ Category: 'Lottery', Metric: 'Total Winners', Value: report.lottery.stats.total_winner_slots || 0 });
    }
    
    if (report?.lottery?.byRewardType) {
      report.lottery.byRewardType.forEach(item => {
        data.push({ 
          Category: 'Lottery Reward Type', 
          Metric: item.lottery_reward_type?.replace('_', ' ') || 'Unknown', 
          Value: item.count,
          PrizePool: item.total_prize_pool || 0
        });
      });
    }
    
    downloadCSV(data, 'votes_lottery_analytics');
  };
  ...
  const handleDownloadVotesPDF = () => {
    ...
    if (report?.lottery?.byRewardType?.length > 0) {
      content.push({ type: 'heading', text: 'Lottery by Reward Type' });
      content.push({
        type: 'table',
        headers: ['Reward Type', 'Count', 'Total Prize Pool'],
        data: report.lottery.byRewardType.map(item => [
          item.lottery_reward_type?.replace('_', ' ') || 'Unknown',
          item.count,
          formatCurrency(item.total_prize_pool)
        ])
      });
    }
    
    downloadPDF('Votes & Lottery Report', content, 'votes_lottery_analytics');
  };
  ...
  const DownloadButtons = ({ onCSV, onPDF, className = "" }) => (
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="bg-gradient-to-r from-purple-700 via-indigo-700 to-blue-700 rounded-2xl p-6 text-white shadow-xl">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <DownloadButtons onCSV={handleDownloadAllCSV} onPDF={handleDownloadAllPDF} />
      ...
      <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl shadow-sm border border-emerald-100 overflow-hidden">
        <button onClick={() => toggleSection('revenue')} className="w-full px-6 py-4 flex items-center justify-between bg-gradient-to-r from-emerald-600 to-teal-600 text-white">
          ...
          <div className="flex items-center gap-3">
            <DownloadButtons onCSV={handleDownloadRevenueCSV} onPDF={handleDownloadRevenuePDF} />
      ...
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl shadow-sm border border-amber-100 overflow-hidden">
        <button onClick={() => toggleSection('platformRevenue')} className="w-full px-6 py-4 flex items-center justify-between bg-gradient-to-r from-amber-600 to-orange-600 text-white">
          ...
          <div className="flex items-center gap-3">
            <DownloadButtons onCSV={handleDownloadPlatformRevenueCSV} onPDF={handleDownloadPlatformRevenuePDF} />
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        <button onClick={() => toggleSection('users')} className="w-full px-6 py-4 flex items-center justify-between bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
          ...
          <div className="flex items-center gap-3">
            <DownloadButtons onCSV={handleDownloadUsersCSV} onPDF={handleDownloadUsersPDF} />
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        <button onClick={() => toggleSection('elections')} className="w-full px-6 py-4 flex items-center justify-between bg-gradient-to-r from-green-600 to-emerald-600 text-white">
          ...
          <div className="flex items-center gap-3">
            <DownloadButtons onCSV={handleDownloadElectionsCSV} onPDF={handleDownloadElectionsPDF} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Vote Analytics */}
        <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
          <div className="px-6 py-4 bg-gradient-to-r from-purple-600 to-violet-600 text-white flex items-center justify-between">
            <div className="flex items-center gap-3"><Vote className="w-6 h-6" /><span className="font-bold text-lg">Vote Analytics</span></div>
            <DownloadButtons onCSV={handleDownloadVotesCSV} onPDF={handleDownloadVotesPDF} />
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
          <div className="px-6 py-4 bg-gradient-to-r from-pink-600 to-rose-600 text-white flex items-center justify-between">
            <div className="flex items-center gap-3"><Trophy className="w-6 h-6" /><span className="font-bold text-lg">Lottery Analytics</span></div>
            <DownloadButtons onCSV={handleDownloadVotesCSV} onPDF={handleDownloadVotesPDF} />
          </div>
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-3 gap-3">
...
//last workable code only to add downloaded pdf above code
// // src/pages/admin/LiveAnalytics.jsx
// // Enhanced with Pie Charts, World Map visualization, and Revenue Stats
// import React, { useState } from 'react';
// import {
